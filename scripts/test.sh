...
if [ "$RUN_E2E" = true ]; then
    ...
    if [ -n "$CELL_CID" ]; then
        ...
        if [ "$CELL_SERVICE" = "cell" ]; then
            ...
            docker compose --profile standard --profile admin --profile email-legacy --profile auditing --profile proxy-rust down 2>/dev/null || true
            NEED_RESTART=true
        fi
    else
        NEED_RESTART=true
    ...
    if [ "$NEED_RESTART" = false ]; then
        ...
        if [ "$ADMIN_MODE" = "connected" ]; then
            ...
            docker compose --profile dev --profile admin --profile email-legacy --profile auditing --profile proxy-rust --profile proxy-legacy down 2>/dev/null || true
            NEED_RESTART=true
        fi
    fi
    ...
    if [ "$NEED_RESTART" = true ]; then
        ...
        docker compose --profile dev --profile admin --profile managed --profile email-legacy --profile auditing --profile proxy-rust --profile proxy-legacy down -v --remove-orphans 2>/dev/null || true
        docker compose --profile standard --profile admin --profile managed --profile email-legacy --profile auditing --profile proxy-rust --profile proxy-legacy down -v --remove-orphans 2>/dev/null || true
