...
# Default DLP patterns shipped with the data plane.
# In standalone mode these are written to dlp_config.json.
# In connected mode the CP sends its own pattern set via GET /api/v1/dlp-policies.
...
class ConfigGenerator:
    ...
    def get_dlp_config(self) -> dict:
        ...
        # Standalone fallback: return sensible defaults with all built-in patterns
        return {
            "enabled": False,
            "mode": "log",
            "skip_domains": [],
            "custom_patterns": list(DEFAULT_DLP_PATTERNS),
        }
