...
def _get_container_resources(container_name: str) -> dict:
    ...
    CPU is read from NanoCpus (set by docker-compose --cpus and our
    low-level update) with CpuQuota/CpuPeriod as fallback.
    """
    result = subprocess.run(
        ["docker", "inspect", "--format",
         "{{json .HostConfig.NanoCpus}} {{json .HostConfig.CpuQuota}} "
         "{{json .HostConfig.CpuPeriod}} {{json .HostConfig.Memory}} "
    ...
    cpu_quota = _safe_int(parts[1]) if len(parts) > 1 else 0
    cpu_period = _safe_int(parts[2]) if len(parts) > 2 else 0
    memory_bytes = _safe_int(parts[3]) if len(parts) > 3 else 0
    pids = _safe_int(parts[4]) if len(parts) > 4 else 0

    # CPU: prefer NanoCpus (set by compose and our update), fall back to quota/period
    if nano_cpus > 0:
