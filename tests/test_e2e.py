...
def _discover_cell_container_name() -> str:
    ...
    try:
        ...
        if _get_compose_project():
            cmd += ["--filter", f"label=com.docker.compose.project={_get_compose_project()}"]
        cmd += ["--format", "{{.Names}}"]
...
class TestDNSFiltering:
    ...
    def test_allowed_domain_resolves(self, data_plane_running):
        ...
        result = exec_in_cell(f"nslookup api.openai.com {DNS_FILTER_IP}")
    ...
    def test_blocked_domain_fails(self, data_plane_running):
        ...
        result = exec_in_cell(f"nslookup evil-malware.com {DNS_FILTER_IP}")
...
class TestLogging:
    ...
    @pytest.mark.skipif(is_rust_mode(), reason="Log format differs in rust proxy mode")
...
class TestCellSecurityHardening:
    ...
    def test_cannot_reach_infra_net(self, data_plane_running):
        ...
        result = exec_in_cell(f"nc -z -w 2 {INFRA_DNS_IP} 53 && echo FAIL || echo BLOCKED")
        assert "BLOCKED" in result.stdout, f"Cell can reach dns-filter on infra-net ({INFRA_DNS_IP})"
...
def get_admin_url():
    ...
    If LOCAL_ADMIN_PORT is set in the environment, use it directly
    instead of probing ``docker port`` (useful when the host-side port
    is overridden by a compose project).
    """
...
class TestLocalAdminConfigPipeline:
    ...
    def test_config_update_propagates_to_cell(self, admin_url, data_plane_running):
        ...
        result = exec_in_cell(f"nslookup {self.TEST_DOMAIN} {DNS_FILTER_IP}")
        ...
        try:
            ...
            subprocess.run(
                ["docker", "restart", f"{_CP_PREFIX}warden"],
                capture_output=True,
                timeout=30,
                check=True,
            )
            assert wait_for_container(f"{_CP_PREFIX}warden", timeout=15), "warden did not restart"
...
@pytest.mark.e2e
@pytest.mark.skipif(is_rust_mode(), reason="Analytics requires Envoy access logs (not generated by cagent-proxy yet)")
class TestAnalytics:
...
# =============================================================================
# VictoriaLogs / Log Ingestion Pipeline
# =============================================================================


def is_log_store_running():
...
class TestDeepHealth:
    """Test /api/metrics health checks including VictoriaLogs liveness."""
    ...
    def test_metrics_health_includes_log_store_check(self, admin_url, data_plane_running):
        ...
        # VL should be healthy if log-store is running.
        if is_log_store_running():
            ...
            while time.time() < deadline:
                ...
                time.sleep(2)
            if data["health"]["checks"]["log_store"]["status"] != "healthy":
                pytest.fail("VictoriaLogs did not become healthy within 90s")
...
@pytest.mark.e2e
@pytest.mark.skipif(is_rust_mode(), reason="Log ingestion pipeline requires Envoy + OpenObserve")
class TestLogIngestionPipeline:
    ...
    Requires log-store + log-shipper running.
    """

    @pytest.fixture(autouse=True)
    def _require_log_store(self, admin_url, data_plane_running):
        """Fail if VictoriaLogs is not running; wait for it to be healthy."""
        if not is_log_store_running():
            pytest.fail("VictoriaLogs not running (log-store container required)")
        # Wait for VL to be healthy before running log ingestion tests
        deadline = time.time() + 90
        while time.time() < deadline:
            try:
                r = requests.get(f"{admin_url}/api/metrics", timeout=10)
    ...
    def test_envoy_logs_ingested_into_vl(self, admin_url, data_plane_running):
        """Proxy traffic should appear in VictoriaLogs via warden search.

        1. Generate HTTP traffic with a unique marker through the proxy
        2. Wait for Vector to ship logs to VictoriaLogs
        3. Query warden /api/logs/search for the marker
        """
        marker = f"vl-e2e-{int(time.time())}"
        ...
        assert found, (
            f"Marker '{marker}' never appeared in VictoriaLogs search within 60s. "
            f"Check Vector → VL pipeline. Last response: {r.status_code} {r.text[:200]}"
        )
...
class TestRuntimeConfig:
    ...
    def test_push_victorialogs_url(self, admin_url, data_plane_running):
        """Push VICTORIALOGS_URL and verify it takes effect."""
        r = requests.post(
            f"{admin_url}/api/commands/update-config",
            json={"config": {"VICTORIALOGS_URL": "http://custom-log-store:9428"}},
            timeout=10,
        )
        assert r.status_code == 200
        assert r.json()["applied"]["VICTORIALOGS_URL"] == "http://custom-log-store:9428"

        r = requests.get(f"{admin_url}/api/commands/runtime-config", timeout=5)
        assert r.json()["effective"]["VICTORIALOGS_URL"] == "http://custom-log-store:9428"
